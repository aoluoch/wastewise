...
router.patch('/:id/start', [authorize('collector'), validate], async (req, res) => {
  try {
  ...
  async (req, res) => {
    try {
      ...
      if (
        ...
        pickupTask.collectorId.toString() !== req.user._id.toString()
      ) {
        return res.status(403).json({
          success: false,
          message: 'Access denied',
...
router.get('/:id', [validate], async (req, res) => {
  try {
    ...
    if (
      req.user.role === 'collector' &&
      pickupTask.collectorId._id.toString() !== req.user._id.toString()
    ) {
      return res.status(403).json({
        success: false,
        message: 'Access denied',
    ...
    if (
      ...
      pickupTask.reportId.userId.toString() !== req.user._id.toString()
    ) {
      return res.status(403).json({
        success: false,
        message: 'Access denied',
...
// @access  Private (Collector)
router.post('/tasks/:id/start', [authorize('collector'), validate], async (req, res) => {
  try {
    const pickupTask = await PickupTask.findById(req.params.id)
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const totalEstimatedDuration = pickupTasks.reduce(
      ...
      const schedule = {
        ...
        status:
          pickupTasks.length === 0
            ? 'pending'
            : pickupTasks.every(task => task.status === 'completed')
              ? 'completed'
              : pickupTasks.some(task => task.status === 'in_progress')
                ? 'in_progress'
                : 'pending',
      }
...
// @access  Private (Admin/Collector)
router.get(
  '/collector/:collectorId/schedule',
  [
    authorize('admin', 'collector'),
...
// @access  Private (Collector)
router.get('/collector/stats', [authorize('collector'), validate], async (req, res) => {
  try {
    ...
    stats.today = {
      ...
      highPriority: todayTasks.filter(task => task.reportId && task.reportId.priority === 'high')
        .length,
    }
    ...
    stats.thisWeek = {
      ...
      completionRate:
        weekTasks.length > 0
          ? Math.round(
              (weekTasks.filter(task => task.status === 'completed').length / weekTasks.length) *
                100
            )
          : 0,
    }
    ...
    try {
      ...
      const performance = {
        ...
        averageCompletionTime:
          completedTasks.length > 0 ? Math.round(totalActualTime / completedTasks.length) : 0,
        efficiency:
          totalEstimatedTime > 0 ? Math.round((totalActualTime / totalEstimatedTime) * 100) : 0,
        tasksByPriority: {
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (
        req.user.role === 'collector' &&
        task.collectorId.toString() !== req.user._id.toString()
      ) {
        return res.status(403).json({
          success: false,
          message: 'Access denied',
