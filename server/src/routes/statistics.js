...
router.get('/resident', [authorize('resident'), validate], async (req, res) => {
  try {
    ...
    for (let i = 29; i >= 0; i--) {
      ...
      const count = reportsLast30Days.filter(
        report => report.createdAt >= startOfDay && report.createdAt < endOfDay
      ).length
