  ...
  async (req, res) => {
    try {
      ...
      if (!aiDecision.allowed) {
        ...
        const confidenceText = aiDecision.confidence
  ...
  async (req, res) => {
    try {
      ...
      if (req.user.role === 'collector') {
        filter.$or = [{ assignedCollectorId: req.user._id }, { status: 'pending' }]
      }
...
// @access  Private
router.get('/feed', [validate], async (req, res) => {
  try {
    const filter = {}
    ...
    const reports = await WasteReport.find(filter)
      ...
      .select(
        'type description images location status priority estimatedVolume notes userId createdAt updatedAt'
      )
      .sort({ createdAt: -1 })
...
router.patch(
  ...
  [
    ...
    body('priority')
      ...
      .isIn(['low', 'medium', 'high', 'urgent'])
      .withMessage('Invalid priority'),
    body('assignedCollectorId').optional().isMongoId().withMessage('Invalid collector ID'),
    body('notes')
      .optional()
      .trim()
      .isLength({ max: 200 })
  ...
  async (req, res) => {
    try {
      ...
      if (
        ...
        report.assignedCollectorId?.toString() !== req.user._id.toString()
      ) {
...
router.delete('/:id', [authorize('admin', 'collector', 'resident'), validate], async (req, res) => {
  try {
    const report = await WasteReport.findById(req.params.id)
    ...
    console.log(
      'Successfully deleted report from database:',
      req.params.id,
      'Deleted count:',
