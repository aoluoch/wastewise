...
const getAdminStats = async () => {
  const [
    ...
    WasteReport.find().populate('userId', 'firstName lastName').sort({ createdAt: -1 }).limit(5),
    PickupTask.find()
...
const getCollectorStats = async userId => {
  const [assignedTasks, completedTasks, pendingTasks, inProgressTasks, todayTasks, upcomingTasks] =
    await Promise.all([
...
const getResidentStats = async userId => {
  const [totalReports, pendingReports, completedReports, recentReports, upcomingPickups] =
    await Promise.all([
      ...
      WasteReport.find({ userId }).sort({ createdAt: -1 }).limit(5),
      PickupTask.find({
...
const getReportAnalytics = async dateFilter => {
  const [totalReports, reportsByType, reportsByStatus, reportsByPriority, dailyReports, topAreas] =
    await Promise.all([
...
const getPickupAnalytics = async dateFilter => {
  const [totalPickups, pickupsByStatus, averageCompletionTime, collectorPerformance, dailyPickups] =
    await Promise.all([
...
const getUserAnalytics = async dateFilter => {
  const [totalUsers, usersByRole, activeUsers, newUsers, userGrowth] = await Promise.all([
