...
export const pickupApi = {
  async getTasks(
    ...
    page = 1,
    limit = 10
  ): Promise<PaginatedResponse<PickupTask>> {
  ...
  async getTask(id: string): Promise<PickupTask> {
    ...
    const taskId =
      ...
      (typeof raw._id === 'string' ? (raw._id as string) : '');

    const reportFromT =
      (raw.report as Record<string, unknown> | undefined) ||
      (raw.reportId as Record<string, unknown> | undefined) ||
      {};
    ...
    if (typeof reportIdRaw === 'string') {
      ...
      reportFromT &&
      typeof reportFromT === 'object' &&
      'id' in reportFromT
    ) {
      reportId =
        typeof (reportFromT as { id?: unknown }).id === 'string'
          ? (reportFromT as { id?: string }).id
          : undefined;
    } else if (
      reportFromT &&
      typeof reportFromT === 'object' &&
      ...
      reportId =
        typeof (reportFromT as { _id?: unknown })._id === 'string'
          ? (reportFromT as { _id?: string })._id
          : undefined;
    }
    ...
    if (typeof collectorIdRaw === 'string') {
      ...
      collectorIdRaw &&
      typeof collectorIdRaw === 'object' &&
      '_id' in collectorIdRaw
    ) {
    ...
    const normalized: PickupTask = {
      ...
      status:
        typeof raw.status === 'string'
          ? (raw.status as PickupTask['status'])
          : 'scheduled',
      scheduledDate:
        typeof raw.scheduledDate === 'string'
          ? (raw.scheduledDate as string)
          : '',
      estimatedDuration:
        typeof raw.estimatedDuration === 'number'
          ? (raw.estimatedDuration as number)
          : 0,
      actualStartTime:
        typeof raw.actualStartTime === 'string'
          ? (raw.actualStartTime as string)
          : undefined,
      actualEndTime:
        typeof raw.actualEndTime === 'string'
          ? (raw.actualEndTime as string)
          : undefined,
      notes: typeof raw.notes === 'string' ? (raw.notes as string) : undefined,
      completionNotes:
        typeof raw.completionNotes === 'string'
          ? (raw.completionNotes as string)
          : undefined,
      images: Array.isArray(raw.images) ? (raw.images as string[]) : [],
      createdAt:
        typeof raw.createdAt === 'string' ? (raw.createdAt as string) : '',
      updatedAt:
        typeof raw.updatedAt === 'string' ? (raw.updatedAt as string) : '',
      report: {
        id:
        ...
        description:
          typeof report.description === 'string'
            ...
            : '',
        location:
          (report.location as unknown as PickupTask['report']['location']) || {
            address: '',
        ...
        priority:
          typeof report.priority === 'string'
            ...
            : 'medium',
        estimatedVolume:
          typeof report.estimatedVolume === 'number'
            ? (report.estimatedVolume as number)
            : 0,
      },
    };
  ...
  async updateTask(id: string, data: Partial<PickupTask>): Promise<PickupTask> {
    const response = await axiosInstance.put<ApiResponse<PickupTask>>(
      `/pickups/tasks/${id}`,
      data
    );
    return response.data.data;
  ...
  async getMyTasks(
    ...
    try {
      ...
      const normalizedTasks = rawTasks.map((raw: unknown) => {
        ...
        const reportFromT =
          (t.report as Record<string, unknown> | undefined) ||
          (t.reportId as Record<string, unknown> | undefined) ||
          {};
        ...
        const normalized: PickupTask = {
          ...
          actualStartTime:
            typeof t.actualStartTime === 'string'
              ? (t.actualStartTime as string)
              : undefined,
          actualEndTime:
            typeof t.actualEndTime === 'string'
              ? (t.actualEndTime as string)
              : undefined,
          notes: typeof t.notes === 'string' ? (t.notes as string) : undefined,
          completionNotes:
            typeof t.completionNotes === 'string'
              ? (t.completionNotes as string)
              : undefined,
          images: Array.isArray(t.images) ? (t.images as string[]) : [],
          createdAt:
            typeof t.createdAt === 'string' ? (t.createdAt as string) : '',
          updatedAt:
            typeof t.updatedAt === 'string' ? (t.updatedAt as string) : '',
          report: {
            id:
              typeof report.id === 'string'
                ? (report.id as string)
                : typeof report._id === 'string'
                  ...
                  : '',
            type:
              typeof report.type === 'string' ? (report.type as string) : '',
            description:
              typeof report.description === 'string'
                ? (report.description as string)
                : '',
  ...
  async getCollectorStats(): Promise<CollectorStats> {
    try {
      ...
      const response = await axiosInstance.get<ApiResponse<CollectorStats>>(
        `/pickups/collector/stats?_=${cacheBuster}`
      );
      return response.data.data;
    } catch {
  ...
  async getCollectorPerformance(
    ...
    const response = await axiosInstance.get<ApiResponse<PerformanceMetrics>>(
      `/pickups/collector/performance?period=${period}`
    );
    return response.data.data;
  },

  // Get resident's pickup schedule (tasks for their reports)
  async getResidentPickups(
    page = 1,
    limit = 20
  ): Promise<PaginatedResponse<PickupTask>> {
    try {
