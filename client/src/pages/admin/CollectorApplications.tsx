...
const CollectorApplications: React.FC = () => {
  ...
  const approveMutation = useMutation({
    mutationFn: async (userId: string) => {
      const response = await axiosInstance.put(`/admin/collector-applications/${userId}/approve`);
      return response.data;
    },
    onSuccess: (_data) => {
      showToast({ type: 'success', message: 'Application approved successfully!' });
      queryClient.invalidateQueries({ queryKey: ['collector-applications'] });
      queryClient.invalidateQueries({ queryKey: ['admin', 'users'] });
    },
    onError: (error: unknown) => {
      const message = (error as { response?: { data?: { message?: string } } }).response?.data?.message || 'Failed to approve application';
  ...
  const rejectMutation = useMutation({
    mutationFn: async ({ userId, reason }: { userId: string; reason?: string }) => {
      const response = await axiosInstance.put(`/admin/collector-applications/${userId}/reject`, {
        reason
      });
      return response.data;
    },
    ...
    onError: (error: unknown) => {
      const message = (error as { response?: { data?: { message?: string } } }).response?.data?.message || 'Failed to reject application';
      showToast({ type: 'error', message });
    }
  });

  const handleApprove = (userId: string) => {
    if (window.confirm('Are you sure you want to approve this application? The user will become a collector.')) {
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-gray-600 dark:text-gray-300">Loading applications...</div>
      </div>
    );
  }
    ...
    <div className="space-y-6 font-['Poppins']">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
        ...
        <p className="text-gray-600 dark:text-gray-300">
          Review and manage applications from residents who want to become collectors
        </p>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
        {applications.length === 0 ? (
          <div className="text-center py-12">
            ...
            <h3 className="mt-2 text-sm font-medium text-gray-900 dark:text-white">No pending applications</h3>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      {application.county || application.constituency ? (
                        <div className="text-sm">
                          {application.county && (
                            <div className="text-gray-900 dark:text-white">{application.county}</div>
                          )}
                          {application.constituency && (
                            <div className="text-gray-500 dark:text-gray-400">{application.constituency}</div>
                          )}
                        </div>
                      ) : (
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-800 rounded-lg p-6 max-w-md w-full mx-4">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-300 mb-4">
              Optionally provide a reason for rejecting this application. This will be sent to the applicant.
            </p>
            <textarea
