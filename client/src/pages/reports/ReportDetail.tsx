...
const ReportDetail: React.FC = () => {
  ...
  const updateMutation = useMutation({
    mutationFn: (payload: { description?: string; notes?: string }) =>
      reportsApi.updateMine(id, payload),
    onSuccess: () => {
    ...
    onError: (error: Error) => {
      showToast({
        message: error.message || 'Failed to update report. Please try again.',
        type: 'error',
      });
    },
  });
            ...
            <div className='space-y-2'>
              ...
              {report.location?.coordinates && (
                <p className='text-sm text-gray-500'>
                  Coordinates: {report.location.coordinates.lat.toFixed(6)},{' '}
                  {report.location.coordinates.lng.toFixed(6)}
                </p>
              )}
            </div>
          ...
          {report.location?.coordinates ? (
            <>
              ...
              <div className='mt-2 text-center'>
                ...
                <p className='text-xs text-gray-400 mt-1'>
                  📍 Exact coordinates:{' '}
                  {report.location.coordinates.lat.toFixed(6)},{' '}
                  {report.location.coordinates.lng.toFixed(6)}
                </p>
              </div>
            </>
            ...
            <div className='bg-gray-100 dark:bg-gray-700 rounded-lg h-96 flex items-center justify-center'>
              <div className='text-center text-gray-500 dark:text-gray-400'>
                ...
                <p className='text-sm'>
                  This report doesn't have location coordinates
                </p>
              </div>
            </div>
          )}
