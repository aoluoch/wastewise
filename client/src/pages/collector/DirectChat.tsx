...
const DirectChat: React.FC = () => {
  ...
  const room =
    !user?.id || !userId ? '' : `dm:${[user.id, userId].sort().join(':')}`;
  ...
  useEffect(() => {
    ...
    userApi
      .getUser(userId)
      .then((u: User) =>
        setPeer({ id: userId, name: `${u.firstName} ${u.lastName}` })
      )
      .catch(() => {
        setPeer({ id: userId, name: 'Collector' });
      });
  }, [userId]);
    ...
    const handleNewMessage = (...args: unknown[]) => {
      ...
      if (message && message.room === room) {
        const normalizedSenderId =
          message.sender?.id || message.senderId || 'system';
        const normalizedSenderName =
          message.sender?.name || message.senderName || 'System';
        const normalizedSenderRole =
          message.sender?.role || message.senderRole || 'system';
        setMessages(prev => [
          ...prev,
          {
            id: message.id,
    ...
    <div className='h-[calc(100vh-12rem)] flex flex-col'>
      ...
      <div className='flex-1 bg-white dark:bg-gray-800 overflow-y-auto p-4 space-y-4'>
        {messages.map(m => (
          <div
            key={m.id}
            className={`flex ${isOwnMessage(m.senderId) ? 'justify-end' : 'justify-start'}`}
          >
            <div
              className={`max-w-xs lg:max-w-md ${isOwnMessage(m.senderId) ? 'bg-blue-600 text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-900 dark:text-white'} rounded-lg px-4 py-2`}
            >
              ...
              <div
                className={`text-xs mt-1 ${isOwnMessage(m.senderId) ? 'text-blue-100' : 'text-gray-500 dark:text-gray-400'}`}
              >
